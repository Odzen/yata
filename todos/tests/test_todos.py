...
def test_checkbox_checked(live_server, page: Page):
    item = TodoItem.objects.create(title="Test item", completed=True)
    page.goto(reverse_url(live_server, "index"))
    id_test_item = f"toggle_item_{item.id}"
    expect(page.get_by_test_id(id_test_item)).to_be_checked()
...
def test_read_items(live_server, page: Page):
...
def test_create_todo_item_gets_rid_of_nothing_to_see(live_server, page: Page):
    ...
    page.get_by_label("Description").fill("foo description")
